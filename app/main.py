...
def get_jellyfin_status():
    """Fetches status and active streams from Jellyfin."""
    url = os.getenv("JELLYFIN_URL", "http://192.168.0.200:8096")
    ...
    try:
        # According to API docs, /Sessions lists active sessions
        api_url = f"{url}/Sessions"
        response = requests.get(api_url, headers={"X-Emby-Token": key}, timeout=1)

        if response.status_code == 200:
            sessions = response.json()
            # Count sessions that have a 'NowPlayingItem'
            active_streams = sum(1 for s in sessions if 'NowPlayingItem' in s)
            return active_streams, "Online"
        else:
...
def render_dashboard():
    ...
    # -- ROW 3: NETWORK & COMMS & JELLYFIN --
    st.subheader("Uplink Status & Services")
    # Updated layout to fit Jellyfin: 3 columns
    net_col, ping_col, jelly_col = st.columns(3)
    ...
    with ping_col:
        with st.container(border=True):
            ...
            # Using colored text for status
            st.markdown(f"**Google:** {'🟢 Online' if google_up else '🔴 Offline'}")
            st.markdown(f"**GitHub:** {'🟢 Online' if github_up else '🔴 Offline'}")

    # Jellyfin Status
    jf_streams, jf_status = get_jellyfin_status()
    jf_color = "off" if jf_status != "Online" else "normal" # CSS styles will handle 'off' vs normal via metric value color

    with jelly_col:
        with st.container(border=True):
            # Display Metric
            # Streamlit metrics don't support custom colors dynamically easily without markdown hacking,
            # but we can rely on our theme.
            st.metric("Jellyfin Status", jf_status, f"{jf_streams} Active Streams")
...
def render_admin_panel():
    with st.expander("⚙️ System Configuration (Secure Vault)"):
        ...
        current_jf_url = os.getenv("JELLYFIN_URL", "http://192.168.0.200:8096")
        ...
        with st.form("secrets_form"):
            ...
            c3, c4 = st.columns(2)
            new_jf_url = c3.text_input("Jellyfin URL", value=current_jf_url)
            new_jf_key = c4.text_input("Jellyfin API Key", value=current_jf_key, type="password")
            ...
            if submitted:
                ...
                save_secrets("JELLYFIN_URL", new_jf_url)
                save_secrets("JELLYFIN_API_KEY", new_jf_key)
